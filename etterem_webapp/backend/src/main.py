...
from flask import jsonify, flash, make_response, request, session, Blueprint
...
def add_user():
    ...
    try:
        _json = request.json
        existing_user = mysql.connect().cursor().execute("SELECT * from user where name=%s", _json['username'])

        if not existing_user:
            _username = _json['username']
            _role_id = mysql.connect().cursor().execute("SELECT id from role where name=%s", _json['role'])
            _password = _json['password']
            ...
            if _username and _role_id and _password and request.method == 'POST':
                ...
                conn = mysql.connect()
                cursor = conn.cursor()
                cursor.execute("INSERT INTO user(username, role_id, password) VALUES(%s, %s, %s)", (_username, _role_id, _hashed_password))
                conn.commit()
...
def get_tablesFoods():
    ...
    try:
        ...
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(
            "select food.id, food.name, food.category_id from food_order join food on food_order.food_id = food.id")
        rows = cursor.fetchall()
        resp = jsonify(rows)
        resp.status_code = 200
        return resp
...
def get_tableFoods(table_order_id):
    ...
    try:
        ...
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(
            "select food.id, food.name, food.category_id, food_order.id as food_order_id, food_order.quantity from food_order join food on food_order.food_id = food.id where table_order_id=%s",
            table_order_id)
        rows = cursor.fetchall()
        resp = jsonify(rows)
        resp.status_code = 200
...
def get_categoryFoods(category_id):
    ...
    try:
        ...
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(
            "select food.id, food.name, food.price, food.category_id from food join category on food.category_id = category.id where category.id = %s",
            category_id)
        rows = cursor.fetchall()
        resp = jsonify(rows)
        resp.status_code = 200
...
def get_roleUsers(role_id):
    ...
    try:
        ...
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(
            "select user.id, user.username, user.role_id from user join role on user.role_id = role.id where role.id = %s",
            role_id)
        rows = cursor.fetchall()
        resp = jsonify(rows)
        resp.status_code = 200
